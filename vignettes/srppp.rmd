...
date: 22 July, 2024 (rebuilt `r Sys.Date()`)
...
Veterinary Office](https://www.blv.admin.ch), now responsible the
...
SRPPP is stored in the package as `srppp::srppp_xml_url`. Reading in the
current data is as simple as
...
between those tables is shown below. 
...
`ingredients`. The P-Numbers contained in the table `pNbrs` are identifiers
of product compositions. Products with the same P-Number are considered
...
Note the use of `dm::dm_flatten_to_tbl` to add information from tables
lower in the table hierarchy (here `substances`) to the `ingredients`
table. The first two lines in the code could also be replaced by
...
which does not make use of the foreign key declaration in the data object.

## Registered products (W-Numbers)

The registered uses are not linked directly to the chemically defined products
(P-Numbers), but to the so-called W-Numbers. The relation between P-Numbers
...
For each product registration (W-Number), the registered uses (tagged 
...
each use by a combination of W-Number (`wNbr`) and use number (`use_nr`).

```{r}
current_register$uses |> 
  filter(wNbr %in% c("5468", "6431") & use_nr < 10) |> 
  select(wNbr, use_nr, ends_with("dosage"), ends_with("rate"), units_de,
...
example_uses <- current_register$products |> 
  filter(wNbr == "6168") |>
  left_join(current_register$uses, by = join_by(wNbr),
...
```{r}
current_register$culture_forms |>
  left_join(current_register$cultures, by = c("wNbr", "use_nr"),
    relationship = "many-to-many") |> 
  filter(wNbr == "6168", use_nr %in% 1:2) |> 
  select(wNbr, use_nr, ends_with("de")) |> 
  kable()
```
...
culture_pest_combinations <- current_register$uses |> 
  filter(wNbr %in% "5468") |> 
  left_join(current_register$cultures, by = c("wNbr", "use_nr")) |> 
  left_join(current_register$pests, by = c("wNbr", "use_nr")) |> 
  select(wNbr, use_nr, application_area_de, culture_de, pest_de)
...
current_register$obligations |>
  filter(wNbr == "6168", use_nr %in% 1:2) |> 
  select(use_nr, code, obligation_de, sw_runoff_points) |> 
  kable()
```
