# Authors and Citation

## Authors

- **Daniel Baumgartner**. Contributor.  
  Provided feedback, explanations and background information

- **Marcel Mathis**. Reviewer, contributor.

- **Romualdus Kasteel**. Reviewer.  
  Provided feedback to version 0.3.4

- **Elisabeth Lutz**. Contributor.

- **Johannes Ranke**. Author, maintainer.
  [](https://orcid.org/0000-0003-4371-6538)

- **Agroscope**. Copyright holder.

## Citation

Source:
[`DESCRIPTION`](https://github.com/agroscope-ch/srppp/blob/v2.0.2/DESCRIPTION)

Ranke J (2026). *srppp: Read the Swiss Register of Plant Protection
Products*. R package version 2.0.2,
<https://agroscope-ch.github.io/srppp/>.

    @Manual{,
      title = {srppp: Read the Swiss Register of Plant Protection Products},
      author = {Johannes Ranke},
      year = {2026},
      note = {R package version 2.0.2},
      url = {https://agroscope-ch.github.io/srppp/},
    }
